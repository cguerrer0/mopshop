...
public class UserBeanTest {
	...
	public void setUp() {
		...
		userBean.setPassword2(PASSWORD);
	...
	public void canAddANewUser() {
		mockery.checking(new Expectations() {{
			allowing(userService).existUser(LOGIN); will(returnValue(false));
			oneOf(userService).addNewCustomer(LOGIN, PASSWORD, CUSTOMER); will(returnValue(new CustomerDTO()));
		}});
	...
	public void updateNewUserAfterAddNewUser() {
		mockery.checking(new Expectations() {
			{
				allowing(userService).existUser(LOGIN); will(returnValue(true));
